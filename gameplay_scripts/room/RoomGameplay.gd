...
var cannot_be_deactivated_manually: bool = false # used by e.g. Cargo Bay and Crew Quarters
...
func init_gameplay_features(data: Dictionary) -> void:
	...
	cannot_be_deactivated_manually = _data.get("cannot_be_deactivated_manually", false)
...
## NOTE: this function also calls remove_power_consumer(), if needed.
func deactivate_room(ignore_power_supplier: bool = false, automatic: bool = false, show_deactivation_notice: bool = false) -> bool:
	...
	match parent_room_type:
		RoomType.CREW_QUARTERS:
			...
			if len(nearest_canteen_data) == 0 or nearest_canteen_data[0].gameplay.activated == false:
				...
				return false
			var all_crew_quarters = get_tree().get_nodes_in_group(str(RoomType.CREW_QUARTERS))
...
func _can_be_deactivated(automatic: bool = false) -> bool:
	...
	if always_activated:
		...
		return false
	if cannot_be_deactivated_manually and not automatic:
...
func _find_sufficient_fuel_storage():
...
func _find_sufficient_ration_storage():
	var adjacent_ration_storages_with_rations = parent_room.adjacent_rooms.filter(func(room: Room): return room.is_in_group(str(RoomType.RATION_STORAGE)) and room.gameplay.rations_remaining > 0)
